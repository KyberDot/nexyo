...
export async function POST(req: NextRequest) {
  const userId = await uid();
  if (!userId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const { sub_id, debt_id, name, mime_type, data, size } = await req.json();
  if (!name || !data) return NextResponse.json({ error: "Name and data required" }, { status: 400 });
  const db = getDb();
  const r = db.prepare("INSERT INTO attachments (user_id, sub_id, debt_id, name, mime_type, data, size) VALUES (?, ?, ?, ?, ?, ?, ?)").run(userId, sub_id || null, debt_id || null, name, mime_type || "application/octet-stream", data, size || 0);
