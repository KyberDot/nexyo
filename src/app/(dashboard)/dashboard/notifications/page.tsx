...
const TYPE_COLORS: Record<string, string> = { renewal: "#6366F1", trial: "#F59E0B", price: "#3B82F6", budget: "#EF4444", system: "#10B981" };
const TYPE_ICONS: Record<string, string> = { renewal: "🔄", trial: "⏰", price: "💲", budget: "🎯", system: "📢" };

// Module-level cache for instant loading
let _notifsCache: any[] | null = null;
let _nsCache: any = null;
let _notifsTime = 0;

export default function NotificationsPage() {
  const [notifs, setNotifs] = useState<any[]>(_notifsCache || []);
  const [ns, setNs] = useState<any>(_nsCache);
  const [tab, setTab] = useState<"inbox" | "settings">("inbox");
  const [filter, setFilter] = useState("all");
  const [loading, setLoading] = useState(!_notifsCache);
  const [savingNs, setSavingNs] = useState(false);
  const [savedNs, setSavedNs] = useState(false);

  const load = async (force = false) => {
    if (!force && _notifsCache && _nsCache && Date.now() - _notifsTime < 30000) { setLoading(false); return; }
    setLoading(true);
    const [nr, nsr] = await Promise.all([fetch("/api/notifications").then(r => r.json()), fetch("/api/notifications/settings").then(r => r.json())]);
    const arr = Array.isArray(nr) ? nr : [];
    _notifsCache = arr; _nsCache = nsr && !nsr.error ? nsr : null; _notifsTime = Date.now();
    setNotifs(arr); if (_nsCache) setNs(_nsCache);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const markRead = async (id?: number) => {
    ...
    setNotifs(prev => {
      const next = id ? prev.map(n => n.id === id ? { ...n, read: true } : n) : prev.map(n => ({ ...n, read: true }));
      _notifsCache = next; return next;
    });
  };

  const deleteNotif = async (id: number) => {
    await fetch("/api/notifications", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    setNotifs(prev => { const next = prev.filter(n => n.id !== id); _notifsCache = next; return next; });
  };

  const clearRead = async () => {
    await fetch("/api/notifications", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clearAll: true }) });
    setNotifs(prev => { const next = prev.filter(n => !n.read); _notifsCache = next; return next; });
  };
        ...
        <>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            {loading ? <div style={{ padding: 40, textAlign: "center", color: "var(--muted)" }}>Loading...</div>
              : filtered.length === 0
                ? <div style={{ padding: 48, textAlign: "center" }}><div style={{ fontSize: 36, marginBottom: 8 }}>🔔</div><div style={{ fontWeight: 600 }}>All clear!</div><div style={{ color: "var(--muted)", fontSize: 13, marginTop: 4 }}>No notifications in this view</div></div>
                : filtered.map((n, i) => (
                  <div key={n.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", borderBottom: i < filtered.length - 1 ? "1px solid var(--border-color)" : "none", background: n.read ? "transparent" : "rgba(var(--accent-rgb),0.03)", transition: "background 0.15s" }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: (TYPE_COLORS[n.type] || "#6366F1") + "15", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>
                      {n.sub_icon ? <img src={n.sub_icon} width={24} height={24} style={{ borderRadius: 4, objectFit: "contain" }} alt="" onError={e => { e.currentTarget.style.display = "none"; }} /> : TYPE_ICONS[n.type] || "📢"}
                    </div>
      ...
      {tab === "settings" && ns && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 560 }}>
          <div className="card">
            ...
            {[["push_enabled", "In-app notifications", "Get notified inside the app"], ["email_enabled", "Email notifications", "Receive emails for important alerts"]].map(([k, label, desc]) => (
              <div key={k} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid var(--border-color)" }}>
                <div><div style={{ fontWeight: 500, fontSize: 14 }}>{label}</div><div style={{ fontSize: 12, color: "var(--muted)", marginTop: 1 }}>{desc}</div></div>
          ...
          <div className="card">
            ...
            {[["renewal_alerts", "Renewal alerts", "Notify before subscriptions renew"], ["price_change_alerts", "Price change alerts", "When subscription prices change"], ["trial_end_alerts", "Trial ending alerts", "Before free trials expire"], ["overdue_alerts", "Overdue alerts", "When a payment becomes overdue"], ["budget_alerts", "Budget alerts", "When spending approaches limits"]].map(([k, label, desc]) => (
              <div key={k} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid var(--border-color)" }}>
                <div><div style={{ fontWeight: 500, fontSize: 14 }}>{label}</div><div style={{ fontSize: 12, color: "var(--muted)", marginTop: 1 }}>{desc}</div></div>
                <Toggle value={!!ns[k]} onChange={v => setNs((p: any) => ({ ...p, [k]: v }))} />
              </div>
          ...
          <button className="btn-primary" onClick={saveSettings} disabled={savingNs} style={{ alignSelf: "flex-start" }}>{savingNs ? "Saving..." : savedNs ? "✓ Saved" : "Save Notification Settings"}</button>
        </div>
      )}
    </div>
  );
