  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Auto-heal missing settings rows to prevent FOREIGN KEY errors
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Use JS ISO string to perfectly match SQLite string comparison
        const now = new Date().toISOString();
        
        const mt = db.prepare("SELECT * FROM magic_tokens WHERE token = ? AND used = 0 AND expires_at > ?").get(credentials.token, now) as any;
        if (!mt) return null;
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      if (token.id && token.role !== "admin") {
        try {
          const db = getDb();
          // Ensure token.id is treated as a number for the DB query
          const u = db.prepare("SELECT active, plan_expires_at FROM users WHERE id = ?").get(Number(token.id)) as any;
          if (!u?.active) {
            token.disabled = true;
          } else if (u.plan_expires_at && new Date(u.plan_expires_at) < new Date()) {
            db.prepare("UPDATE users SET active = 0 WHERE id = ?").run(Number(token.id));
            token.disabled = true;
          }
        } catch {}
      }
